...
<html lang='en'>
    ...
    <body class="background">
        ...
        <?php
            ...
            $response = $_GET['resp'];
            $factid = $_GET['id'];
            $respid = generate_uuid();
            $uuid = $_SESSION['uuid'];
        
            if ($response == "like"){
                $query = "INSERT INTO responses (responseid, factid, type, uuid) VALUES ('$respid', '$factid', 'like', '$uuid')";
                $rows = mysql_query($query) or die(mysql_error());
                
                $query = "UPDATE facts SET likes = likes + 1 WHERE factid = '$factid'";
                $rows = mysql_query($query) or die(mysql_error());
            }elseif ($response == "dislike"){
                $query = "INSERT INTO responses (responseid, factid, type, uuid) VALUES ('$respid', '$factid', 'dislike', '$uuid')";
                $rows = mysql_query($query) or die(mysql_error());
                
                $query = "UPDATE facts SET dislikes = dislikes + 1 WHERE factid = '$factid'";
                $rows = mysql_query($query) or die(mysql_error());
            }
        
            echo '<script>window.location.href = "facts.php?id='.$factid.'"</script>';
