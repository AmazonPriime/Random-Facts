    ...
    <body class="background">
        ...
        <div>
            <?php
                $link = connectdb();
                $uuid = $_SESSION['uuid'];
                $text = mysql_real_escape_string($_POST['fact']);
                $category = mysql_real_escape_string($_POST['type']);
                $factid = generate_uuid();
                
                $query = "INSERT INTO factqueue (factid, text, category, author) VALUES ('$factid', '$text', '$category', '$uuid')";
